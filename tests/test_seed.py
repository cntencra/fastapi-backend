...
@pytest.mark.asyncio
async def test_table_column_data_type(db_manager: DBManager, table_name: str, column_name: str, data_type:str):
    async with db_manager.get_conn() as conn:
...
@pytest.mark.asyncio
async def test_table_has_data(db_manager: DBManager, table_name: str, columns: list, test_data_length: int):
    async with db_manager.get_conn() as conn:
        async with conn.cursor(row_factory=dict_row) as cur:
