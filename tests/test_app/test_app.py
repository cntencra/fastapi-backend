...
async def test_get_visits(async_client: AsyncClient):
    ...
    for visit in response.json():
        assert isinstance(visit["visit_id"], int)
        assert isinstance(visit["visit_name"], str)
        assert isinstance(visit["visit_date"], str)